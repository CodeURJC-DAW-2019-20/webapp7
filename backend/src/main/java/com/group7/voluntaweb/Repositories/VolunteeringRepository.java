...
public interface VolunteeringRepository extends JpaRepository<Volunteering, Long> {
	...
	@Modifying
	@Transactional
	@Query(value="DELETE FROM like WHERE user_id = :user_id AND volunteering_id = :volunteering_id", nativeQuery = true)
	void deleteLike(@Param("user_id") long user_id, @Param("volunteering_id") long volunteering_id);
	//
	@Modifying
	@Transactional
	@Query(value="INSERT INTO like WHERE user_id = :user_id AND volunteering_id = :volunteering_id", nativeQuery = true)
	void addLike(@Param("user_id") long user_id, @Param("volunteering_id") long volunteering_id);
	
	@Query("SELECT like FROM Like like WHERE volunteering.id = :volunteeringId AND user.id = :userId")
	Like findLike(@Param("volunteeringId") long volunteeringId, @Param("userId") long userId);
